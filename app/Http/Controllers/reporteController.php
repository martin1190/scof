...
class reporteController extends Controller
{
    ...
    public function resumenprocedimiento(Request $request){
        $rp=DB::select('select p.id, c.nconsulta, p.nombre, p.edad, p.sexo, pr.procedimiento from consulta c, paciente p, procedimientos pr, datoprevio dp
            where p.id=c.paciente_id and pr.consulta_id=c.id and dp.consulta_id=c.id and dp.fechacon between :fi and :ff ',['fi'=>$request->fi, 'ff'=>$request->ff]);
    ...
    public function cantidadProcedimiento(Request $request){
        $r=DB::select('select pr.procedimiento, count(pr.procedimiento) as cantidad from procedimientos pr, consulta c, datoprevio dp where c.id=pr.consulta_id and dp.consulta_id=c.id and dp.fechacon between :fi and :ff group by pr.procedimiento',['fi'=>$request->fi,'ff'=>$request->ff]);
    ...
    public function PDFResumenCantidadPr($fi, $ff){
        $r=DB::select('select pr.procedimiento, count(pr.procedimiento) as cantidad from procedimientos pr, consulta c, datoprevio dp where c.id=pr.consulta_id and dp.consulta_id=c.id and dp.fechacon between :fi and :ff group by pr.procedimiento',['fi'=>$fi,'ff'=>$ff]);
    ...
    public function detalleEdades(Request $request){
        $ed=DB::table('paciente')
                ...
                ->whereBetween('datoprevio.fechacon',[$request->fi,$request->ff])->get();
